...
#define DEFAULT_TRIAL_LENGTH 300
#define DEFAULT_NUM_TRIALS 40
...
int main(int argc, char* argv[]) {
   // usage: $ poker [numTrials [trialLength]] 
   int numTrials = argc > 1 ? atoi(argv[1]) : DEFAULT_NUM_TRIALS;
   int trialLength = argc > 2 ? atoi(argv[2]) : DEFAULT_TRIAL_LENGTH;
   ...
   for (i = 0; i < numTrials; i++) {
      ...
      if (result->value() == 52) {
         ...
         //cout << "New best at " << result->value() << ": " << *result << "\n";
         delete bestSoFar;
         bestSoFar = result;
      } else {
         delete result;
